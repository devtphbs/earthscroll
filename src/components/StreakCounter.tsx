'use client'

import { useState, useEffect } from 'react'
import { User } from '@supabase/supabase-js'

interface StreakCounterProps {
  user: User | null
}

export function StreakCounter({ user }: StreakCounterProps) {
  const [streak, setStreak] = useState(0)
  const [loading, setLoading] = useState(true)

  useEffect(() => {
    const loadStreak = async () => {
      setLoading(true)
      
      if (user) {
        // TODO: Load from Supabase
        // For now, use localStorage as fallback
        const savedStreak = localStorage.getItem('streak')
        const lastVisit = localStorage.getItem('lastVisit')
        
        if (savedStreak && lastVisit) {
          const streakData = parseInt(savedStreak)
          const lastVisitDate = new Date(lastVisit)
          const today = new Date()
          const yesterday = new Date(today)
          yesterday.setDate(yesterday.getDate() - 1)
          
          if (lastVisitDate.toDateString() === today.toDateString()) {
            setStreak(streakData)
          } else if (lastVisitDate.toDateString() === yesterday.toDateString()) {
            const newStreak = streakData + 1
            setStreak(newStreak)
            localStorage.setItem('streak', newStreak.toString())
            localStorage.setItem('lastVisit', today.toISOString())
          } else {
            setStreak(1)
            localStorage.setItem('streak', '1')
            localStorage.setItem('lastVisit', today.toISOString())
          }
        } else {
          setStreak(1)
          localStorage.setItem('streak', '1')
          localStorage.setItem('lastVisit', new Date().toISOString())
        }
      } else {
        // Anonymous user - use localStorage only
        const savedStreak = localStorage.getItem('streak')
        const lastVisit = localStorage.getItem('lastVisit')
        
        if (savedStreak && lastVisit) {
          const streakData = parseInt(savedStreak)
          const lastVisitDate = new Date(lastVisit)
          const today = new Date()
          const yesterday = new Date(today)
          yesterday.setDate(yesterday.getDate() - 1)
          
          if (lastVisitDate.toDateString() === today.toDateString()) {
            setStreak(streakData)
          } else if (lastVisitDate.toDateString() === yesterday.toDateString()) {
            const newStreak = streakData + 1
            setStreak(newStreak)
            localStorage.setItem('streak', newStreak.toString())
            localStorage.setItem('lastVisit', today.toISOString())
          } else {
            setStreak(1)
            localStorage.setItem('streak', '1')
            localStorage.setItem('lastVisit', new Date().toISOString())
          }
        } else {
          setStreak(1)
          localStorage.setItem('streak', '1')
          localStorage.setItem('lastVisit', new Date().toISOString())
        }
      }
      
      setLoading(false)
    }

    loadStreak()
  }, [user])

  if (loading) {
    return (
      <div className="glass-panel-tinted px-3 py-1 rounded-full">
        <div className="w-8 h-4 bg-white/20 rounded animate-pulse"></div>
      </div>
    )
  }

  return (
    <div className="flex items-center space-x-1 glass-panel-tinted px-3 py-1 rounded-full">
      <svg 
        className="w-4 h-4 text-orange-400 animate-pulse-glow" 
        fill="currentColor" 
        viewBox="0 0 20 20"
      >
        <path fillRule="evenodd" d="M12.395 2.553a1 1 0 00-1.45-.385c-.345.23-.614.558-.822.88-.214.33-.403.713-.57 1.116-.334.804-.614 1.768-.84 2.734a31.365 31.365 0 00-.613 3.58 2.64 2.64 0 01-.945-1.067c-.328-.68-.398-1.534-.398-2.654A1 1 0 005.05 6.05 6.981 6.981 0 003 11a7 7 0 1011.95-4.95c-.592-.591-.98-.985-1.348-1.467-.363-.476-.724-1.063-1.207-2.03zM12.12 15.12A3 3 0 017 13s.879.5 2.5.5c0-1 .5-4 1.25-4.5.5 1 .786 1.293 1.371 1.879A2.99 2.99 0 0113 13a2.99 2.99 0 01-.879 2.121z" clipRule="evenodd" />
      </svg>
      <span className="text-sm font-medium text-white">{streak}</span>
    </div>
  )
}
